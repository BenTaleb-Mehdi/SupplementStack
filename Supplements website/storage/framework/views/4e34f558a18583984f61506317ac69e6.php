<?php $__env->startSection('title', 'Secure Checkout - Mini Market'); ?>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="cardholder_name" name="cardholder_name" value="<?php echo e(Auth::user()->name); ?>" class="w-full rounded-xl border-gray-200 focus:border-primary-500 focus:ring-primary-500 bg-gray-50 focus:bg-white transition-colors py-3" required>
                            </div>

                            <div class="md:col-span-2">
                                <label for="shipping_address" class="block text-sm font-medium text-gray-700 mb-2">Shipping Address</label>
                                <textarea id="shipping_address" name="shipping_address" rows="3" class="w-full rounded-xl border-gray-200 focus:border-primary-500 focus:ring-primary-500 bg-gray-50 focus:bg-white transition-colors py-3" required><?php echo e(Auth::user()->address); ?></textarea>
                            </div>

                            <div>
                                <label for="phone" class="block text-sm font-medium text-gray-700 mb-2">Phone Number</label>
                                <input type="tel" id="phone" name="phone" value="<?php echo e(Auth::user()->phone); ?>" class="w-full rounded-xl border-gray-200 focus:border-primary-500 focus:ring-primary-500 bg-gray-50 focus:bg-white transition-colors py-3" required>
                            </div>

                             <div>
                                <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Email Address</label>
                                <input type="email" value="<?php echo e(Auth::user()->email); ?>" class="w-full rounded-xl border-gray-200 bg-gray-100 text-gray-500 py-3 cursor-not-allowed" readonly disabled>
                            </div>
                        ...
                        <div class="p-6">
                                        ...
                                        <li class="flex py-4">
                                            <div class="h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg border border-gray-200">
                                                <?php if($item['product']->image): ?>
                                                    <img src="<?php echo e($item['product']->image_url); ?>" alt="<?php echo e($item['product']->name); ?>" class="h-full w-full object-cover object-center" onerror="this.onerror=null; this.src='<?php echo e(asset('images/placeholder.svg')); ?>';">
                                                <?php else: ?>
                                            ...
                                            <div class="ml-4 flex flex-1 flex-col">
                                                <div>
                                                    <div class="flex justify-between text-base font-medium text-gray-900">
                                                        <h3 class="line-clamp-1"><a href="#"><?php echo e($item['product']->name); ?></a></h3>
                                                        <p class="ml-4"><?php echo e(App\Models\Setting::formatPrice($item['subtotal'])); ?></p>
                                                    </div>
                                                    <p class="mt-1 text-sm text-gray-500"><?php echo e(ucfirst($item['product']->category)); ?></p>
                                                </div>
                            ...
                            <div class="border-t border-gray-100 mt-6 pt-6 space-y-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <p class="font-medium text-gray-900"><?php echo e(App\Models\Setting::formatPrice($total)); ?></p>
                                </div>
                                <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center justify-between border-t border-gray-100 pt-4">
                                    ...
                                    <p class="text-xl font-bold text-primary-600"><?php echo e(App\Models\Setting::formatPrice($total)); ?></p>
                                </div>
                            </div>
                        </div>
...
<script>
    // Initialize Stripe
    const stripe = Stripe('<?php echo e(env('STRIPE_KEY', 'pk_test_demo')); ?>');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            if (paymentMethod === 'cod') {
                const response = await fetch('<?php echo e(route('payment.cod')); ?>', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    window.location.href = `<?php echo e(route('checkout.success')); ?>?order=${result.order_id}`;
                } else {
                    throw new Error(result.error || 'Failed to process order');
                }
            } else {
                // Card Payment
                const intentResponse = await fetch('<?php echo e(route('payment.create-intent')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
                ...
                if (paymentIntent.status === 'succeeded') {
                     const confirmResponse = await fetch('<?php echo e(route('payment.confirm')); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
                    ...
                    if (confirmResult.success) {
                        window.location.href = `<?php echo e(route('checkout.success')); ?>?order=${confirmResult.order_id}`;
                    } else {
...
<?php echo $__env->make('layouts.main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\LENOVO\CascadeProjects\Market-Store - v2\mini-market\resources\views/checkout/index.blade.php ENDPATH**/ ?>