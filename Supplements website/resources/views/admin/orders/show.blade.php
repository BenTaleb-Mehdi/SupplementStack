...
<div class="space-y-6">
    <!-- Back Button -->
    <div>
        <a href="{{ route('admin.orders.index') }}" class="inline-flex items-center text-primary-600 hover:text-primary-700 font-medium">
                ...
                <div class="p-6">
                            ...
                            <div class="flex items-center space-x-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                    @if($item->product && $item->product->image)
                                        <img src="{{ $item->product->image_url }}" alt="{{ $item->product->name ?? 'Product' }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-500">Price: {{ App\Models\Setting::formatPrice($item->price) }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-semibold text-gray-900">{{ App\Models\Setting::formatPrice($item->subtotal) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-2xl font-bold text-primary-600">{{ App\Models\Setting::formatPrice($order->total_amount) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    @if($order->phone)
                        <div class="bg-primary-50 border border-primary-200 rounded-lg p-3">
                            ...
                            <p class="text-base text-primary-900 font-bold">{{ $order->phone }}</p>
                        </div>
                    @else
                    ...
                    @if($order->shipping_address)
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
                            ...
                            <p class="text-sm text-blue-900 leading-relaxed">{{ $order->shipping_address }}</p>
                        </div>
                    @else
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-3">
                    ...
                    @if($order->notes)
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                            ...
                            <p class="text-sm text-yellow-900 leading-relaxed">{{ $order->notes }}</p>
                        </div>
                    @endif
                </div>
            </div>
